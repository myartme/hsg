...
const { pdfMeta, pdfListElement, isOpenPdfOptions, isOpenNightOrder, isOpenBootlegger, isSavedScript, isWaitingOperation, pdfListWithParams, isEditingScript, nightOrderAutoGenerated, isNightOrderMismatch } = storeToRefs(craftStore)
...
const tooltipClipboard = computed(() => {
  return t('tooltips.copyToClipboard')
})
const tooltipPdf = computed(() => {
  return t('tooltips.downloadPdf')
})
const tooltipJson = computed(() => {
  return t('tooltips.downloadJson')
})
